...
from data_provider.presto_reaction_tasks_cot_stringonly_dataset import PrestoCoTReaction
import re
...
def smiles_handler(text, is_gal=True, graph_only=False):
    smiles_list = []
...
class TrainCollater:
    def __init__(self, tokenizer, text_max_len, is_gal=True, graph_only=False):
    ...
    def __call__(self, batch):
        ...
        ## deal with prompt
        smiles_prompt = [smiles_handler(p, self.is_gal, self.graph_only)[0] for p in smiles_prompt]
...
class PrestoCoTStringOnlyDM(LightningDataModule):
    def __init__(
        self,
        mode: str = 'pretrain',
        num_workers: int = 0,
        ...
        self.train_dataset = PrestoCoTReaction("train", shuffle=True)
        self.val_dataset = PrestoCoTReaction("valid", shuffle=False, n_test_samples=args.n_test_samples)
        self.test_dataset = PrestoCoTReaction("test", shuffle=False, n_test_samples=args.n_test_samples)
    ...
    def train_dataloader(self):
        ...
        loader = DataLoader(
            ...
            collate_fn=TrainCollater(self.tokenizer, self.text_max_len, self.is_gal, self.graph_only),
        )
        return loader
    ...
    def val_dataloader(self):
        ...
        val_loader_2 = DataLoader(
            ...
            drop_last=False,
            persistent_workers=True,
            collate_fn=InferenceCollater(self.tokenizer, self.text_max_len, self.is_gal, self.graph_only),
        )
        ...
        return [val_loader, test_loader, val_loader_2]
