...
def parse_output(raw_response):
    ...
    answer_text = extract_answer_content(raw_response).replace(", ", ".")
...
if os.path.exists("CoT_experiments/data/fewshot_example_all_tmp.json"):
    with open("CoT_experiments/data/fewshot_example_all_tmp.json", "r") as f:
    ...
    for task_name in task_names:
        ...
        for seed, idx in enumerate(n_shot_indices[task_name][:7]):
            for i in idx:
                ...
                user_prompts[task_name][seed].append(data[i]['user_prompt'])
...
with open(f"CoT_experiments/data/openai_batch/requests/retro_test.jsonl", "w") as f:
...
jsonl_files = [
    f"CoT_experiments/data/openai_batch/requests/retro_test.jsonl",
]
...
retro_catalyst_file = f"CoT_experiments/data/openai_batch/responses/retro_test.jsonl"
# retro_catalyst_test.jsonl 데이터를 읽어서 custom_id를 키로 하는 딕셔너리 생성
retro_catalyst_data = {}
with open(retro_catalyst_file, "r", encoding="utf-8") as rc_file:
    for line in rc_file:
        record = json.loads(line)
        retro_catalyst_data[record["custom_id"]] = record
...
for reasoning_file in reasoning_files:
    # reasoning_test.jsonl 데이터를 읽으면서 custom_id가 일치하면 데이터 교체
    merged_data = []
    with open(reasoning_file, "r", encoding="utf-8") as rm_file:
        for line in rm_file:
            record = json.loads(line)
            custom_id = record.get("custom_id")
            # custom_id가 일치하면 데이터를 교체, 그렇지 않으면 그대로 사용
            if custom_id in retro_catalyst_data:
                merged_data.append(retro_catalyst_data[custom_id])
            else:
                merged_data.append(record)

    # 결과를 새로운 파일로 저장
    with open(reasoning_file.replace(".jsonl", "_tmp.jsonl"), "w", encoding="utf-8") as output:
