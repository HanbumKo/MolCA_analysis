...
for model in ["gpt-4o-mini"]:
    ...
    for i, task in enumerate(task_names):
        for j, metric in enumerate(metrics):
            ...
            # 박스 색상 설정 (짝수 index → w/o, 홀수 index → w/)
            colors = []
            for k in range(len(all_box_data)):
                if k % 2 == 0:
                    colors.append('#1f77b4')  # 파랑 (w/o)
                else:
                    colors.append('#ff7f0e')  # 주황 (w/)
    ...
    axes[0, 0].legend(
        ...
        ["w/o reasoning", "w/ reasoning"],
        loc="upper left"
    )

    plt.tight_layout()
    plt.savefig(f"CoT_experiments/results/cot_prompt_test/plots/{model}_test_box.png", dpi=300)
    plt.clf()
    ...
    lines = [
        plt.Line2D([0], [0], color=reasoning_styles["w/o reasoning instruction"]["color"],
                marker=reasoning_styles["w/o reasoning instruction"]["marker"], label="w/o reasoning"),
        plt.Line2D([0], [0], color=reasoning_styles["w/ reasoning instruction"]["color"],
                marker=reasoning_styles["w/ reasoning instruction"]["marker"], label="w/ reasoning")
    ]
    # fig.legend(handles=lines, loc='upper center', ncol=2)
    axes[0, 0].legend(handles=lines, loc='upper left')

    plt.tight_layout()
    plt.savefig(f"CoT_experiments/results/cot_prompt_test/plots/{model}_test_line.png", dpi=300)
    plt.clf()
