...
def get_openai_output(data_dict, task_name, model="gpt-3.5-turbo"):
    if task_name == "forward":
        system_message = """You are a chemistry expert, and your task is to generate a step-by-step explanation of a chemical forward reaction prediction process, which involves predicting possible products from precursors composed of reactants and reagents. Specifically, you should follow these steps:
    ...
    5.	Derive the product in SMILES notation.

The user will provide the following information:
    - Precursor (SMILES format)
...
It would be nice if you could explain it by relating the existing substructure to the chemical reaction mechanism. Using all the information provided, you must synthesize a coherent, logical paragraph that follows the steps outlined above."""
