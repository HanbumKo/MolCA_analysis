...
def main(args):
    ...
    if args.opt_model.find('galactica') >= 0 or args.opt_model.find('t5') >= 0 or args.opt_model.find('only') >= 0:
    ...
    if len(args.devices.split(',')) > 1:
        ...
        if args.val_check_interval < 1.0:
            trainer = Trainer(accelerator=args.accelerator, devices=args.devices, precision=args.precision, max_epochs=args.max_epochs, val_check_interval=args.val_check_interval, accumulate_grad_batches=args.accumulate_grad_batches, callbacks=callbacks, strategy=strategy, logger=logger)
        else:
...
def get_args():
    ...
    parser.add_argument('--val_check_interval', type=float, default=100.0)
